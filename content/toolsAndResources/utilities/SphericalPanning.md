...
var addSphericalPanningCameraToScene = function (scene, canvas) {
  ...
  var pan = (currX, currY) => {
    ...
    var getPointerViewSpaceDirectionToRef = (x, y, ref) => {
      BABYLON.Vector3.UnprojectToRef(new BABYLON.Vector3(x, y, 0), canvas.width, canvas.height, BABYLON.Matrix.Identity(), BABYLON.Matrix.Identity(), camera.getProjectionMatrix(), ref);
      ref.normalize();
    };
