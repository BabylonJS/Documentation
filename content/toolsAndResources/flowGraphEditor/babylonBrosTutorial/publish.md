...
In the editor, save to the snippet server. You get an ID back; the final tutorial graph is `#YAJ8CD`. The scene reference is stored alongside it, so reopening the editor restores the preview.
...
```javascript
const coordinator = await BABYLON.ParseFlowGraphCoordinatorFromSnippetAsync("YAJ8CD", { scene });
coordinator.start();
```
...
- Round-trip your edits through the snippet server to compare against `#YAJ8CD`.
