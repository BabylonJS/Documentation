...
The image above shows three types of nodes available in the graph, an input node, a source node, and an output node. At minimum all graphs need exactly one output node and at least one source node. Note that the inputs and outputs on the nodes share colors and icons that give hints on what can be connected. Since the `Geometry Output` node has only a geometry input, there needs to be at least one node supplying that that input.
...
The second method is to press the spacebar for a shortcut to open a search window with a the full node list at the mouse position on the graph. Once the window is open, we can simply start typing to enter a keyword in the search bar. Once the list is filtered, use either the mouse to click the desired node or use the arrow keys to highlight the desired node and press enter. This will drop the node at the mouse position when the shortcut was invoked. 

![Press spacebar for shortcut to search bar to filter node list which appears at the mouse position](/img/tools/nge/nodeAddShortcut.jpg)
...
- **Node Materal ID** is used to display a node material on the preview mesh in the editor. To use a node material, it must be saved to the snippet server and the snippet ID is placed in this field. 
