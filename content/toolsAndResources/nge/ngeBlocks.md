...
Note that when manipulating the position of instances with matrix input or position input, the positions generated by this node will be overwritten. To avoid this, use the position contextual value node as the basis for position before modification to retain the positions generated by this node. 
...
### Optimize
There are times when a geometry will have unwelded vertices where mutliple vertices occupy the same position. This is common when using glTF files as the specification states that vertices that lie on a UV seam should be split for optimization reasons. If working with geometry that has unwelded vertices that need to be merged, use the optimize node. This node does a simple operation that merges any vertices that share the same position within a certain epsilon. This operation is destructive in that it creates a new triangle list and deletes UV coordinates from vertices as they would no longer be valid with a new triangle list. Any vertices that have been merged will be given a new vertex normal that is the average of the merged vertex normals.
