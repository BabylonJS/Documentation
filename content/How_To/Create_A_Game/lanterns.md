...
Then, we extract the mesh from the root, and remove the root. [The coordinate system guide for 3DMax](/workflow/wfDeeper/Exporters/3DSMax_to_glTF#left-to-right-handed-coordinate-system) explains why we have this root node when we import the glTF. Since we want to clone just the mesh, we'll need to remove the root.
