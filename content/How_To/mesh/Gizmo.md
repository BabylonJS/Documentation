...
Note: Gizmos will set/modify the [rotationQuaternion](/features/divingDeeper/mesh/transforms) of the attached node. After attaching, any rotation of the mesh should be done with the rotationQuaternion property instead of rotation.
...
Gizmos are displayed by a [UtilityLayerRenderer](/features/divingDeeper/mesh/utilityLayerRenderer) to not disrupt the existing scene state. If not specified, the default utility layer will be used.
...
These gizmo's internally use a [pointerDragBehavior](/features/divingDeeper/behaviors/meshBehaviors), this is exposed and can be used perform tasks before/during/after dragging a gizmo
...
To drag around objects contained inside a bounding box, [Mesh Behaviors](/features/divingDeeper/behaviors/meshBehaviors) can be attached.
...
UI can be attached to the bounding box using the [AttachToBoxBehavior](/features/divingDeeper/behaviors/meshBehaviors)
