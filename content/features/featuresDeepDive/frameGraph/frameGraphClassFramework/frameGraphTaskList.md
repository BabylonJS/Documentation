...
<H3Image title="FrameGraphMeshBlendingTask" image="/img/how_to/meshBlending/coastal-after.png" alt="Coastal cliff scene with mesh blending"/>

Task which softens validated contacts between meshes by blending SceneColor in screen space.

[Link to the class](/typedoc/classes/babylon.framegraphmeshblendingtask)

[Mesh blending documentation](/features/featuresDeepDive/postProcesses/meshBlending)

<Playground id="#BJBKKU#12" image="/img/how_to/meshBlending/coastal-after.png" title="Mesh blending coastal cliff" description="Coastal cliff scene using the FrameGraph mesh blending task" isMain={false}/>
<Playground id="#BJBKKU#14" image="/img/how_to/meshBlending/coastal-after.png" title="Mesh blending coastal cliff (NRG)" description="Coastal cliff scene loading the mesh blending Node Render Graph with ParseFromSnippetAsync" isMain={false}/>

Inputs:
* **sourceTexture**. SceneColor to blend.
* **meshBlendTagTexture**. Packed `R8UI` mesh tags generated by `FrameGraphGeometryRendererTask`.
* **depthTexture**. View or screen depth aligned with SceneColor and the mesh tags.
...
* **slopeFactor**. Contact-slope narrowing factor. A value of `1` disables narrowing.
* **depthType**. Representation stored in **depthTexture**.
* **debugMode**. Compiled diagnostic visualization.
* [postProcess](/typedoc/classes/babylon.thinmeshblendingpostprocess). The shared thin post-process implementation.
<br/>
