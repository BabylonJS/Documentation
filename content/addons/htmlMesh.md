---
title: HtmlMesh
image: public/img/playgroundsAndNMEs/htmlMeshPG.webp
description: The HTML Mesh is a mesh that displays a webpage (or DOM element) in the scene, meaning that it can occlude other meshes and be occluded by other meshes.
further-reading:
video-overview:
video-content:
---
...
The HTML Mesh is a mesh that displays a webpage (or DOM element) in the scene, meaning that it can occlude other meshes and be occluded by other meshes.
...
<Playground id="#B17TC7#210" title="HtmlMesh Box Example" description="Example of using multiple HtmlMesh instances as the sides of a box"  image="/img/playgroundsAndNMEs/HtmlMeshBoxPG.webp"/>

The following use cases are supported by the HtmlMesh:
...
Note that the use of HtmlMesh requires that the experience be accessed through a browser, so in its
...
have them appear with placeholder content or perhaps even an option to view the raw HTML in a popup UI.

By default, HtmlMesh instances are "in scene," meaning that they can be occluded by objects in the scene and may occlude objects in the scene when they are between the camera and the object. In-scene HtmlMesh instances require the scene clear color to be transparent, and they must be rectangular. HtmlMesh instances can also be created as "overlays." In this case, they render in front of the scene, can be semi-transparent, and can be non-rectangular. However, overlays will render in front of all scene content, even content that is between the mesh and the camera. Overlays are a good choice when you want an HTML overlay to be attached to an object in the scene, as they eliminate the need to handle projection and transformation of the HTML element yourself.

By default, the HtmlMesh will capture pointer events as soon as the pointer enters. This is meant to make it easier for users to interact with site content without requiring an extra click or other gesture. Note that pointer capture won't occur when a camera zoom causes the pointer to be over the mesh. This allows users to zoom in and out of the scene without the mesh capturing the pointer and preventing zoom as soon as the pointer enters. This behavior can be disabled as described in the [pointer capture](#pointer-capture) section below, in which case you will need to provide a mechanism to trigger pointer capture and release in your code. Note that on Android devices, the pointer will be captured and the event dispatched to the underlying element on first touch. However, due to the way iOS manages touch events, on iOS the user will need to touch the mesh to capture pointer events before the events will be sent to the element. For example, to play a YouTube video, two touches will be required.

## Installation

HtmlMesh is available as a UMD NPM package
...
The first step is to create an instance of `HtmlMeshRenderer`. Pass the scene, and optionally an options object containing:
...
Next, create the DOM element for your content. This can be any HTML element, though most of the time it should either be a `div` for DOM content in the same app or an `iframe` for external DOM content. You should not add this element to your document; `HtmlMesh` will do this for you. Set any attribute and style values that you want; however, be aware that the width and height styles will be replaced by the `HtmlMesh`.

One thing to be aware of is that the way the scale is determined can sometimes result in the element being larger than the mesh if the mesh has a substantial difference between its world min and max z values. If this is the case, you may want to wrap your element in an outer div that is slightly larger and has a background color. This will ensure that any gaps between the mesh and the element are filled with the background color, and the user can access the entire portion of the element that needs to be accessible. A future update may add support for this to `setContent`.

Create your HtmlMesh instance by specifying the id and, optionally, an options object containing:

- `captureOnPointerEnter` - Specifies whether the HtmlMesh should capture pointer events whenever the pointer is over the mesh. Defaults to true. If false, you will need to manually capture and release pointer events for the
...
- `fitStrategy` - Specifies how the HTML element should scale to fit the HtmlMesh's size. Fit behavior is like CSS `background-size`.
  - FitStrategy.NONE Default. By default, the HTML element will have a width and height that fill the viewport while respecting the aspect ratio of the element, and it will then be scaled down to fit the HtmlMesh size specified in `setContent`. This provides the best possible text quality when using iframes and is the recommended strategy for iframes.
  - FitStrategy.CONTAIN Scales the HTML element as large as possible within its container without cropping or stretching (i.e. changing the element aspect ratio). The main difference between this and none is that this strategy (and all strategies other than none) will wrap the element with a sizing and scaling container that fills the mesh. This allows the element to be sized and aligned independently of the mesh. The downside to this and all strategies other than none is that it can result in upscaling at some zoom levels, which can impact text quality for iframes. For non-iframes, this should not be an issue because the font size can be adjusted to maximize text clarity.
...
Finally, call `setContent`, passing in the element and the mesh width and height in BabylonJS units. Be aware that any scaling done after `setContent` will not be preserved on the next call to `setContent`. You should capture any scaling you want preserved and pass the scale values through `setContent`. See [Scaling `HtmlMesh` Instances](#scaling-htmlmesh-instances) for an explanation of why this is the case. You can set attributes and styles after calling `setContent` using a query selector on the id. The `HtmlMesh` can be positioned, oriented, parented, shown, or hidden like any other mesh. You can even use pointer drag behavior and gizmos to allow users to position and move the mesh, subject to the caveats about scaling below.
...
In order to keep the DOM content and mesh size synced, the DOM content size will be adjusted every frame using CSS transforms to match the mesh. This means that if the mesh is scaled (using, say, a bounding box gizmo), the content will be scaled as well. Since the content is set after the mesh is created, the mesh initially starts as a plane of size 1. Once the content is set, we need to scale the mesh to match. We can't just scale the mesh, because that would then scale the content. So we compute a scaling transform and bake it into the mesh vertices, effectively rewriting the mesh size. We also store an inverse scale matrix that can reverse the scale. The next time `setContent` is called, we will reverse the scale to bring the mesh back to 1 and then compute a new scale transform (and inverse) and bake it into the mesh vertices. The consequence of this is that any scaling applied (via, say, the bounding box gizmo) will be reverted on the next call to `setContent`. So if you are storing information to recreate HtmlMesh instances on a server, you will want to make sure that you update the original size with any scaling changes so that the next time you call `setContent`, they will be preserved.
...
An HtmlMesh instance can be configured to automatically capture the pointer on entry. This allows for "one-click" behavior where the user can interact directly with the content. For example, the user can click the play button directly on a YouTube video. Iframes are problematic, however, because we cannot detect pointer movement within a cross-origin iframe (an iframe that originates from a different site than the one hosting it). Simply put, we can capture the pointer, but releasing it requires the pointer to leave the iframe completely. This requirement means that we cannot detect when the pointer is over a mesh that is in front of the HtmlMesh containing the iframe; only when the pointer has fully left the iframe will the pointer be detected. In practice, since BabylonJS relies on pointer events on the canvas, this means that no actions, pointer observables, or scene picking will work correctly if the pointer entered the iframe first, until the pointer has left the iframe. Therefore, if you have mesh content that needs to be pickable (respond in some way to pointer events) and could potentially be in front of an HtmlMesh containing an iframe (either due to camera orientation or movement of either mesh), we recommend one of the following:
