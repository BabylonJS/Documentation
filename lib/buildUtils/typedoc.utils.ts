...
export const getTypeDocBasePath = (baseLocation: string = "typedoc") => path.join(process.cwd(), ".temp", baseLocation, "docdirectory");

export const getTypeDocFilesPath = (baseLocation: string = "typedoc") => path.join(getTypeDocBasePath(baseLocation), "files");

export const hasTypeDocArtifacts = (baseLocation: string = "typedoc") => existsSync(path.join(getTypeDocFilesPath(baseLocation), "index.html"));

export const assertTypeDocArtifacts = (baseLocation: string = "typedoc") => {
    if (!hasTypeDocArtifacts(baseLocation)) {
        throw new Error(`TypeDoc artifacts are missing for '${baseLocation}'. Run 'npm run build:typedoc' before building or rendering API routes.`);
    }
};
...
export const getAPIPageData = async (id: string[], baseLocation: string = "typedoc") => {
    assertTypeDocArtifacts(baseLocation);
...
export interface TypeDocSearchEntry {
    name: string;
    kind: string;
    url: string;
}

export interface TypeDocSearchManifestEntry {
    module: string;
    file: string;
    count: number;
}

export interface TypeDocSearchArtifacts {
    byModule: Record<string, TypeDocSearchEntry[]>;
    manifest: TypeDocSearchManifestEntry[];
    legacyRedirects: Record<string, string>;
}

export const createTypeDocSearchArtifacts = (basePathResolved: string, baseLocation: string): TypeDocSearchArtifacts => {
    const filesDir = path.join(basePathResolved, "files");
    const byModule: Record<string, TypeDocSearchEntry[]> = {};
    ...
    for (const entries of Object.values(byModule)) {
        for (const entry of entries) {
            ...
            if (!legacyRedirects[key]) {
                legacyRedirects[key] = entry.url;
            }
        }
    }

    return { byModule, manifest, legacyRedirects };
};

export const writeTypeDocSearchArtifacts = (artifacts: TypeDocSearchArtifacts, baseLocation: string, includeLegacyRedirects: boolean = true, outRoot: string = path.join(process.cwd(), "public", "api-search")) => {
    const prefix = baseLocation.replace(/\//g, "-");
    const outDir = path.join(outRoot, prefix);
    mkdirSync(outDir, { recursive: true });

    for (const manifestEntry of artifacts.manifest) {
        const moduleEntries = artifacts.byModule[manifestEntry.module] || [];
        writeFileSync(path.join(outDir, manifestEntry.file), JSON.stringify(moduleEntries));
    }

    writeFileSync(path.join(outDir, "manifest.json"), JSON.stringify(artifacts.manifest));

    if (includeLegacyRedirects) {
        writeFileSync(path.join(outDir, "legacy-redirects.json"), JSON.stringify(artifacts.legacyRedirects));
        console.log(`Legacy redirect map: ${Object.keys(artifacts.legacyRedirects).length} entries`);
    }

    const total = artifacts.manifest.reduce((sum, manifestEntry) => sum + manifestEntry.count, 0);
    console.log(`Search index: ${total} entries across ${artifacts.manifest.length} modules → ${outDir}/`);
};

export const buildTypeDocSearchArtifacts = (basePathResolved: string, baseLocation: string, options: { includeLegacyRedirects?: boolean } = {}) => {
    const artifacts = createTypeDocSearchArtifacts(basePathResolved, baseLocation);
    writeTypeDocSearchArtifacts(artifacts, baseLocation, options.includeLegacyRedirects !== false);
    return artifacts;
};
