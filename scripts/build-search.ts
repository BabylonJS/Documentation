...
import { buildApiSearchIndex } from "../lib/buildUtils/typedocSearchIndex.utils";
...
async function main() {
	...
	const apiSearchIndex = buildApiSearchIndex();

	console.log(`Wrote ${documentationSearchIndex.length} documentation search item(s), ${apiSearchIndex.items.length} API search item(s) and ${playgroundSearchIndex.length} playground search item(s).`);
}
